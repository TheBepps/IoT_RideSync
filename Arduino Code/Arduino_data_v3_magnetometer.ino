...
BLECharacteristic ble_mag("12345678-1234-1234-1234-1234567890FF", BLERead | BLENotify, 20);
...
int16_t mag_x_int, mag_y_int, mag_z_int;
float x, y, z;
float u, v, w;
float mag_x, mag_y, mag_z;
char acceleration[20];
char gyroscope_data[20];
char magnetometer[30];
...
// Mean magnetometer data
float sum_mag_x = 0;
float sum_mag_y = 0;
float sum_mag_z = 0;
float count_mag = 0;
float mean_mag_x, mean_mag_y, mean_mag_z;
...
int last_mag_send_time = 0;
int last_gyro_send_time = 0;

int last_INA_send_time = 0;

#define env_send_period  20000  // 20 seconds
#define acc_send_period 2000   // 2 seconds
#define mag_send_period 2000   // 2 seconds
#define gyro_send_period 2000  // 2 seconds

#define INA_send_period 1000   // 1 second
...
void setup() {
  ...
  AccService.addCharacteristic(ble_mag);
  ...
  ble_acc.writeValue("0 0 0");
  ble_mag.writeValue("0 0 0");
  ble_gyro.writeValue("0 0 0");



  ble_INA.writeValue("0000 0000 0000");
  ble_INA2.writeValue("0000 0000 0000");
...
void loop() {
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      // Sum contributions of magnetometer data to compute the mean value before sending BLE data
      accumulate_mag();
...
void send_acc_data() {
  ...
  if ((current_time - last_acc_send_time) > acc_send_period) {
    ...
    snprintf(acceleration, sizeof(acceleration), "%d %d %d", x_int, y_int, z_int);
...
void send_mag_data() {
  ...
  if ((current_time - last_mag_send_time) > mag_send_period) {
  
    // Mean magnetometer
    mean_mag_x = sum_mag_x/count_mag;
    mean_mag_y = sum_mag_y/count_mag;
    mean_mag_z = sum_mag_z/count_mag;
    mag_x_int = int(round(mean_mag_x*1000));
    mag_y_int = int(round(mean_mag_y*1000));
    mag_z_int = int(round(mean_mag_z*1000));
    snprintf(magnetometer, sizeof(magnetometer), "%d %d %d", mag_x_int, mag_y_int, mag_z_int);

    // Write values to BLE characteristics
    ble_mag.writeValue(magnetometer);

    // Update last_acc_send_time
    last_mag_send_time = current_time;
    ...
    // Serial print
    Serial.println("Sending magnetometer data "); 
...
void send_gyro_data() {
  ...
  if ((current_time - last_gyro_send_time) > gyro_send_period) {
    ...
    snprintf(gyroscope_data, sizeof(gyroscope_data), "%d %d %d", uu_int, v_int, w_int);
...
void send_INA_data() {
  ...
  if ((current_time - last_INA_send_time) > INA_send_period) {
    ...
    snprintf(INA_meas, sizeof(INA_meas), "%d %d %d", int(volt_TEG), int(volt_prod_mV), int(curr_TEG));
    snprintf(INA2_meas, sizeof(INA2_meas), "%d %d %d", int(volt_batt_mV), int(curr_consumption), int(curr_panel));
...
// Function to accumulate magnetometer data
void accumulate_mag() {
  // new values
  IMU.readMagneticField(mag_x, mag_y, mag_z); 
  // sum contributions
  sum_mag_x = sum_mag_x + mag_x;
  sum_mag_y = sum_mag_y + mag_y;
  sum_mag_z = sum_mag_z + mag_z;
